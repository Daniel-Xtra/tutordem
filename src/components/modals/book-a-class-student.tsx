...
const BookAClassStudent = ({
  ...
  type FieldName = keyof ClassBookingFormValues;

  const handleAddSelectedDay = () => {
    if (selectedDay && selectedTime) {
      // Check if day already exists
      const dayExists = selectedDays.some(d => d.day === selectedDay);
      if (dayExists) {
        return; // Don't add duplicate days
      }
      ...
      form.setValue('days', [...selectedDays, newDay]);
  ...
  const handleRemoveDay = (dayToRemove: string) => {
    const updatedDays = selectedDays.filter(d => d.day !== dayToRemove);
        ...
        description={
          <>
            <span className="text-xs/[16.8px] font-normal font-sans text-neutral-600">
              Let&rsquo;s book a class for {name}
            </span>
          </>
        }
        primaryFn={() => {}}
                ...
                {currentStep === 0 && (
                  <>
                    ...
                    <div className="col-span-12 md:col-span-3 mt-2">
                      <Button
                        type="button"
                        className="bg-primary-500 py-5 px-8 h-14 rounded-sm font-sans w-full font-semibold text-sm/[19.6px] text-[#FFFFFF]"
                        onClick={handleAddSelectedDay}
                      >
                        Add
                      </Button>
                    </div>
                    <div className="col-span-12 flex flex-col w-full space-y-2 text-xs/[16.8px]">
                        {selectedDays.map((days) => (
                            <div key={days.day} className="flex flex-row items-center justify-between bg-primary-25 p-4 rounded-sm border border-primary-500">
                                ...
                                <div className="flex items-center gap-x-2">
                                    ...
                                    <button 
                                      type="button"
                                      onClick={() => handleRemoveDay(days.day)}
...
export default BookAClassStudent;
